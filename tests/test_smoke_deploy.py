...
class SmokeDeployTests(unittest.TestCase):
    ...
    def test_smoke_can_print_account_diagnostics(self):
        def fake_urlopen(request, timeout):
            path = request.full_url.replace("http://service", "")
            if path == "/health":
                return FakeHTTPResponse(
                    200,
                    {
                        "ok": True,
                        "models": ["gemini-3.1-flash-lite", "gemini-3.5-flash", "gemini-3.1-pro"],
                        "accounts": {"total": 2, "available": 0, "current_account_id": None},
                        "auth": {"api_key_required": True},
                    },
                )
            if path == "/v1/models" and not request.headers.get("Authorization"):
                raise urllib.error.HTTPError(
                    request.full_url,
                    401,
                    "Unauthorized",
                    {"X-Request-ID": "req-401"},
                    BytesIO(json.dumps({"error": {"message": "missing key"}}).encode("utf-8")),
                )
            if path == "/v1/models":
                return FakeHTTPResponse(
                    200,
            ...
            if path == "/v1/generation-readiness":
                return FakeHTTPResponse(
                    200,
                    {
                        "ok": True,
                        "ready": False,
                        "reasons": [{"code": "no_available_accounts"}],
                        "accounts": {
                            "total": 2,
                            "available": 0,
                            "disabled": 1,
        ...
        self.assertIn("generation readiness ok", results)
        self.assertIn(
            "account diagnostics: total=2, available=0, disabled=1, expired=1, current=-, ready=False, reasons=no_available_accounts",
            results,
        )
        ...
        def fake_urlopen(request, timeout):
            ...
            if path == "/v1/chat/completions" and request.get_method() == "OPTIONS":
                ...
                self.assertEqual(
                    ...
                    "authorization,x-api-key,api-key,openai-api-key,content-type",
                )
    ...
    def test_smoke_rejects_cors_preflight_without_api_key_alias_headers(self):
        def fake_urlopen(request, timeout):
            path = request.full_url.replace("http://service", "")
            if path == "/health":
                return FakeHTTPResponse(
        ...
        self.assertIn("missing x-api-key", str(raised.exception))
    ...
    def test_smoke_rejects_chat_completion_unexpected_model(self):
        def fake_urlopen(request, timeout):
            path = request.full_url.replace("http://service", "")
            if path == "/health":
                return FakeHTTPResponse(
    ...
    def test_smoke_rejects_chat_stream_unexpected_model(self):
        def fake_urlopen(request, timeout):
            path = request.full_url.replace("http://service", "")
            if path == "/health":
                return FakeHTTPResponse(
