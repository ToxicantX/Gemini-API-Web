...
`/v1/responses` 也支持新版 `tools` / `tool_choice` / `parallel_tool_calls` 入参；非流式响应如果触发工具，会在 `output` 中返回 `function_call` 项，包含 `call_id`、`name` 和 JSON 字符串形式的 `arguments`，方便外部 Responses API 客户端继续执行工具。
